...
- Proposed-and-reviewed cleanup, scene-hull architecture (v0.8.x, judgment-tour + subject-first specs): `cleanup_scene` (chat quick-action or any natural cleanup phrasing) runs an app-owned `CleanupController` — statistical subject floor (robust voxel density + streak/scale filter) → app-flown outline survey where the model boxes the ACTUAL scene per view (`outline_scene`, Qwen 0-1000 grounding coords) → reprojection carves the keep-set (reasoning leads, stats bound it) → delete-set tint + ONE `keep_only_subject` card with a looser/tighter slider → gridded noise survey (`mark_noise`) → lock-in merges marks with statistical voxel clusters → per-cluster judgment tour (fly-to-frame + tint + one forced `judge_candidate` call) → ONE `delete_clusters` batch card with reviewed rows → approved deletions execute per cluster with silhouette verify. Model failures degrade to safe defaults (circuit breaker after 2 consecutive), captures/poses are watchdogged, Stop hard-cancels the run task. Freeform Clean chat uses the v0.5 gated loop for targeted edits (kinds: delete-selection, keep-only-selection, bulk-edit); the crop-box flow was DELETED in v0.8.2 — the manual crop gizmo in the left rail is the only box tool
...
- **Cleanup-tour perception untuned**: the judgment-tour machinery is fully tested headlessly (`backend/agent/tests/test_cleanup_controller.py`), but live-model tuning on real drone scenes remains — grid size (4×4 vs 3×3), tint visibility, and the mark/judge instruction phrasing for the local Qwen-VL
