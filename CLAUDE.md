...
- Proposed-and-reviewed cleanup, judgment-tour architecture (v0.7, judgment-tour-cleanup spec): `cleanup_scene` (chat quick-action or typed) runs an app-owned `CleanupController` — app-computed crop card (operator gizmo binds the box) → app-flown gridded survey where the model marks noise cells (`mark_noise`, one forced call per frame) → lock-in merges marks with statistical voxel clusters → per-cluster judgment tour (fly-to-frame + tint + one forced `judge_candidate` call; junk/structure/look_closer) → ONE `delete_clusters` batch card with reviewed rows → approved deletions execute per cluster with silhouette verify. Any model failure degrades to unsure=keep; the run cannot stall. Freeform Clean chat still uses the v0.5 gated loop (kinds: crop-outside-box, delete-selection, keep-only-selection, bulk-edit; approvals bind the exact reviewed operation). Camera moves during review do NOT pause the run; Stop cleanly ends it
